...
function CustomHooksPage() {
  ...
  useEffect(() => {
    fetch(useInputMd).then((data) => data.text()).then((text) => {
      setMd1(text);
    });
    fetch(useTabsMd).then((data) => data.text()).then((text) => {
      setMd2(text);
    });
    fetch(useTitleMd).then((data) => data.text()).then((text) => {
      setMd3(text);
    });
    fetch(useClickMd).then((data) => data.text()).then((text) => {
      setMd4(text);
    });
    fetch(useConfirmMd).then((data) => data.text()).then((text) => {
      setMd5(text);
    });
    fetch(useBeforeLeaveMd).then((data) => data.text()).then((text) => {
      setMd6(text);
    });
    fetch(useFadeInMd).then((data) => data.text()).then((text) => {
      setMd7(text);
    });
    fetch(useNetworkMd).then((data) => data.text()).then((text) => {
      setMd8(text);
    });
    fetch(useFullscreenMd).then((data) => data.text()).then((text) => {
      setMd9(text);
    });
  }, []);
  ...
  return (
    <>
      ...
      <HookBlock md={md5} ExampleComponent={SampleUseConfirmAndUsePreventLeaveComponent}/>
